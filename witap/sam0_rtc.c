...
static struct device *debug_dev;
...
{
	gpio_pin_set(debug_dev, DT_GPIO_GPSIO_HOST_INT2_GPIOS_PIN, 1);
	while (RTC0->STATUS.reg & RTC_STATUS_SYNCBUSY) {
		/* Wait for bus synchronization... */
	}
	gpio_pin_set(debug_dev, DT_GPIO_GPSIO_HOST_INT2_GPIOS_PIN, 0);
}
...
u32_t rtc_announce(void)
{
	...
	if (count != rtc_last) {
		ticks = (count - rtc_last) / CYCLES_PER_TICK;
...
int rtc_init(void)
{
	...
	debug_dev = device_get_binding(DT_GPIO_GPSIO_HOST_INT1_GPIOS_CONTROLLER);
	gpio_pin_configure(debug_dev, DT_GPIO_GPSIO_HOST_INT1_GPIOS_PIN, GPIO_DIR_OUT);
	gpio_pin_set(debug_dev, DT_GPIO_GPSIO_HOST_INT1_GPIOS_PIN, 0);
	gpio_pin_configure(debug_dev, DT_GPIO_GPSIO_HOST_INT2_GPIOS_PIN, GPIO_DIR_OUT);
	gpio_pin_set(debug_dev, DT_GPIO_GPSIO_HOST_INT2_GPIOS_PIN, 0);
	...
	// u16_t ctrl = RTC_MODE0_CTRL_MODE(0) | RTC_MODE0_CTRL_PRESCALER(DT_INST_0_ATMEL_SAM0_RTC_PRESCALER);
	u16_t ctrl = RTC_MODE0_CTRL_MODE(0) | RTC_MODE0_CTRL_PRESCALER(RTC_MODE0_CTRL_PRESCALER_DIV32);
#ifndef CONFIG_TICKLESS_KERNEL
...
#ifdef CONFIG_TICKLESS_KERNEL
	int key = irq_lock();
	u32_t ret = (rtc_count() - rtc_last) / CYCLES_PER_TICK;
	irq_unlock(key);
	return ret;
#else
