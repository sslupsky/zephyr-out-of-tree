...
uint8_t CayenneLPP::addGPS(uint8_t channel, int32_t latitude, int32_t longitude, int32_t meters)
{
  if ((_cursor + LPP_GPS_SIZE) > _maxsize)
  {
    _error = LPP_ERROR_OVERFLOW;
    return 0;
  }

  int32_t lat = latitude / 1000;      //  SAM-M8Q returns lat and lon as integers x 10^7.  LPP is x 10^4
  int32_t lon = longitude / 1000;
  int32_t alt = meters / 10;          //  SAM-M8Q returns altitude in mm (m x 10^3).  LPP is in cm (m x 10^2)
